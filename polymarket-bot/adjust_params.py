...
def main():
    ...
    if args.sl is not None:
        print(f"Changing Stop Loss: {conf.get('stop_loss_pct')} -> {args.sl}")
        conf["stop_loss_pct"] = args.sl
        updated = True
        
    if args.edge is not None:
        print(f"Changing Min Edge: {conf.get('min_edge')} -> {args.edge}")
        conf["min_edge"] = args.edge
        updated = True
        
    if args.margin is not None:
        print(f"Changing Safety Margin: {conf.get('safety_margin_pct')} -> {args.margin}")
        conf["safety_margin_pct"] = args.margin
        updated = True
        
    if updated:
        save_config(conf)
        print("🚀 Changes will be hot-reloaded by the bot in <60s.")
    else:
        parser.print_help()
